...
export default function LoanHistory({ loans = [], onChanged }) {
  ...
  const LoanRow = ({ loan }) => {
    ...
    return (
      <div className={`flex items-center gap-3 px-4 py-3 transition hover:bg-[rgba(255,255,255,0.03)] ${isPaid ? 'opacity-50' : ''}`}>
        {/* Type icon */}
        <span
          className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-base ${
            isGiven ? 'bg-emerald-500 bg-opacity-15' : 'bg-red-500 bg-opacity-15'
          }`}
        >
          {isGiven ? '💰' : '💸'}
        </span>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            {isPaid && (
              <span className="shrink-0 rounded-full bg-emerald-500 bg-opacity-20 px-1.5 py-0.5 text-[10px] font-semibold text-emerald-300">
                settled
              </span>
            )}
          </div>
        ...
        <div className="flex shrink-0 gap-1">
          {!isPaid && (
            <button
              ...
              className="rounded-lg px-2 py-1 text-[10px] font-semibold text-emerald-400 border border-emerald-500 border-opacity-30 hover:bg-emerald-500 hover:bg-opacity-10 transition disabled:opacity-40"
            >
  ...
  const Section = ({ title, icon, items, totalLabel, totalValue, totalClass }) => (
    <div>
      <div className={`border-b border-white border-opacity-10 px-4 py-3 flex items-center justify-between`}>
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-white border-opacity-10 bg-[rgba(255,255,255,0.04)] backdrop-blur-sm">
      {/* Header */}
      <div className="border-b border-white border-opacity-10 bg-gradient-to-r from-[rgba(124,58,237,0.3)] to-[rgba(192,38,211,0.3)] px-6 py-4">
