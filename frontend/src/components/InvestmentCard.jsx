...
const TYPE_COLORS = {
  Stocks: 'bg-blue-500 bg-opacity-20 text-blue-300',
  'Mutual Funds': 'bg-purple-500 bg-opacity-20 text-purple-300',
  ETFs: 'bg-cyan-500 bg-opacity-20 text-cyan-300',
  Cryptocurrency: 'bg-amber-500 bg-opacity-20 text-amber-300',
  Gold: 'bg-yellow-500 bg-opacity-20 text-yellow-300',
  Silver: 'bg-slate-400 bg-opacity-20 text-slate-300',
  'Savings Certificates': 'bg-emerald-500 bg-opacity-20 text-emerald-300',
  'Real Estate': 'bg-orange-500 bg-opacity-20 text-orange-300',
  'Fixed Deposits': 'bg-teal-500 bg-opacity-20 text-teal-300',
  Bonds: 'bg-indigo-500 bg-opacity-20 text-indigo-300',
  Other: 'bg-white bg-opacity-10 text-slate-300',
};
...
export default function InvestmentCard({ summary = {}, portfolio = [] }) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-white border-opacity-10 bg-[rgba(255,255,255,0.04)] backdrop-blur-sm">
      {/* Header */}
      <div className="border-b border-white border-opacity-10 bg-gradient-to-r from-[rgba(5,150,105,0.3)] to-[rgba(13,148,136,0.3)] px-6 py-4">
      ...
      {typeEntries.length > 0 && (
        <div className="border-t border-white border-opacity-10 px-6 py-4">
          <p className="mb-2 text-[10px] uppercase tracking-wide text-slate-400">Asset Distribution</p>
          <div className="flex flex-wrap gap-1.5">
            {typeEntries.map(([type, amt]) => (
              <span key={type} className={`rounded-full px-2 py-0.5 text-xs font-medium ${TYPE_COLORS[type] || 'bg-white bg-opacity-10 text-slate-300'}`}>
                {type} · {formatPKR(amt)}
              </span>
            ))}
          </div>
      ...
      {active.length > 0 && (
        <div className="border-t border-white border-opacity-10">
          <div className="max-h-52 overflow-y-auto divide-y divide-white divide-opacity-5">
            {active.map(inv => {
              const pl = Number(inv.profit_loss);
              const plCol = pl >= 0 ? 'text-emerald-400' : 'text-red-400';
              return (
                <div key={inv.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-[rgba(255,255,255,0.03)] transition">
                  <span className={`rounded px-1.5 py-0.5 text-[9px] font-semibold ${TYPE_COLORS[inv.type] || 'bg-white bg-opacity-10 text-slate-300'}`}>
                    {inv.type}
                  </span>
